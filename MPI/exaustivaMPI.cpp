...
int main(int argc, char *argv[]) {
    ...
    int partSize = grafoPacote.numVertices / size;
    int start = rank * partSize;
    int end = (rank == size - 1) ? grafoPacote.numVertices : (rank + 1) * partSize;

    vector<int> cand(partSize);
    iota(cand.begin(), cand.end(), start);
